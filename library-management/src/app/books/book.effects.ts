...
export class BookEffects {
    ...
    addBook$ = createEffect(() => this.actions$.pipe(
        ...
        ofType(bookActions.AddBook), //Run when AddBook action appears

        // For each 'AddBook' action, call 'addBook' on the book service.
        // 'mergeMap' allows multiple concurrent 'addBook' calls.
        mergeMap((action) => this.bookService.addBook(action) //Merge observables into single observable
            .pipe(
                // If the 'addBook' call is successful, dispatch 'AddBookSuccess' action with the book data.
                map(book => bookActions.AddBookSuccess(book)), //Take book and start action
                // If the 'addBook' call fails, dispatch 'AddBookFailure' action with the error.
                catchError((error) => of(bookActions.AddBookFailure({ error }))) //If error
            ))
    ))
